...
class Matcher(filter: String, val rootLocation: String = new File(".").getCanonicalPath,
              checkSubdirectory: Boolean = false, contentFilter: Option[String] = None) {

  val rootIOObject = FileConverter.convertToIOObjects(new File(rootLocation))
  ...
  def execute() = {
    ...
    val matchedFiles = rootIOObject match {
      ...
      case directory: DirectoryObject =>
        if (checkSubdirectory) recursiveMatch(directory.children(), List())
