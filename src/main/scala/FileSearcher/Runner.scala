...
object Runner extends App {
  ...
  val matcher = args match {
    ...
    case Array(filter, rootPath, checkSubDirectories) => new Matcher(filter, rootPath,
      convertToBool(checkSubDirectories))
    case Array(filter, rootPath, checkSubDirectories, contentFilter, _*) =>
      new Matcher(filter, rootPath, convertToBool(checkSubDirectories), Some(contentFilter))
  }
